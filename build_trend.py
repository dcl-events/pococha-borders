...
<div class="app">
  ...
  <div class="card">
    ...
    <div class="charthint">日付をなぞると数値を表示 ／ タップで早見表のその日へ移動</div>
  </div>
...
function showTip(e){
  ...
  tip.innerHTML = `<span class="tipd">${p.d}（${wd}）</span><span class="tipv">${fmtMan(p.v)}</span>`;
  tip.style.opacity=1;
}
function hideTip(){
  tip.style.opacity=0;
...
function jump(e){
  const n = nearest(e); if(!n) return;
  location.href = "../?date=" + n.p.iso + "&ct=" + state.ct;   // 早見表(index.html)の該当日へ
}
